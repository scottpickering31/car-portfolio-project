...
function CalendarDisplay({
  ...
  const calculatePrices = (startDate, endDate) => {
    ...
    if (days >= 31) {
      setTarriffType("Gold Tier Achieved!");
      ...
      setTarriffType("Silver Tier Achieved!");
      newPrices = Array.from({ length: days }, (_, index) => ({
        date: new Date(startDate.getTime() + index * oneDay)
          .toISOString()
          .split("T")[0],
      ...
      setTarriffType("Bronze Tier Achieved!");
      newPrices = Array.from({ length: days }, (_, index) => ({
        date: new Date(startDate.getTime() + index * oneDay)
          .toISOString()
          .split("T")[0],
          ...
          <div className="flex flex-col items-center w-full">
            <div className="p-5 bg-customBlue text-white font-semibold tracking-widest rounded-lg w-full shadow-2xl mobile:text-lg mobile:p-5 mobilelg:text-lg mobilelg:p-5 tablet:text-lg tablet:p-5 laptop:text-sm laptop:p-0 desktop:text-lg desktop:p-3">
              ...
              <div className="flex flex-col justify-center gap-2 items-center">
                <p className="font-bold bg-customBlue-lighter w-11/12 rounded-full p-2 mobile:text-xl mobilelg:text-xl tablet:text-2xl laptop:text-xl desktop:text-xl">
                  £{total}
                </p>
                <p>
            ...
            <img
              className="hover:scale-110 transition duration-300 cursor-pointer w-2/3 mobile:h-full mobile:p-5 mobilelg:h-1/2 mobilelg:p-5 tablet:p-2 tablet:h-1/2 laptop:p-2 laptop:h-1/2 desktop:p-1 desktop:h-1/2 widescreen:w-1/4"
